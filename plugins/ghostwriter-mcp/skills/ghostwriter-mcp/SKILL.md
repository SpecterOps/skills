...
```bash
GHOSTWRITER_MCP_SOURCE=<git-url-or-local-path> plugins/ghostwriter-mcp/scripts/install-mcp-deps.sh
```
...
2. If the server is missing, tell the user to run `scripts/install-mcp-deps.sh` with `GHOSTWRITER_MCP_SOURCE` or `--source`.
...
- Missing server directory: run `scripts/install-mcp-deps.sh` or set `GHOSTWRITER_MCP_DIR`.
